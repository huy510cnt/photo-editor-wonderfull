...
public class AviaryConfigActivity extends BaseSlidingFragmentActivity {
	...
	private static final String API_SECRET = "5b9da8803e5ed01c";
	...
	private static final String FOLDER_NAME = "PhotoEditorWonder";
	
	private static final String FOLDER_EDITTED_PATH = "mnt/sdcard/PhotoEditorWonder";
	...
	public void onReload(){
		...
		if(menuFragment instanceof PhotoEditorObserver){
			((PhotoEditorObserver)menuFragment).update();
		}
	...
	public void showConfirmDialog(final String filePath){
	...
	public void startFeather( String absolutePath ) {
...
//		File file = getNextFileName();
		...
		if ( null != file ) {
			mOutputFilePath = absolutePath.substring(0,(absolutePath.lastIndexOf(".")))+"_"+System.currentTimeMillis()+".jpg";
			Log.d("AbsolutePath = ", ""+mOutputFilePath);
		} else {
...
//		 } );
	...
	private class HDAsyncTask extends AsyncTask<Cursor, Integer, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			progress_ = new ProgressDialog( AviaryConfigActivity.this );
		...
		protected void onPostExecute( String errorString ) {
			...
			if ( null != errorString ) {
				Toast.makeText( AviaryConfigActivity.this, "There was an error: " + errorString, Toast.LENGTH_SHORT ).show();
				return;
			}
			...
			new AlertDialog.Builder( AviaryConfigActivity.this ).setTitle( "File saved" )
		...
		private boolean loadImage( MoaHD moa ) throws AviaryExecutionException {
			final String srcPath = IOUtils.getRealFilePath( AviaryConfigActivity.this, uri_ );
			if ( srcPath != null ) {
